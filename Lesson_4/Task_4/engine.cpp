...
void userInput() {
  ...
  // loop for drawing with lines
  for (int i = 0; i < segments - 1; i++)
  {
    double angleFirst = 360 / segments;
    double segmentFirstX = centreX + radius * cos(M_PI * angleFirst * i / 100);
    double segmentFirstY = centreY + radius * sin(M_PI * angleFirst * i / 100);
    double angleSecond = 360 / segments;
    double segmentSecondX = centreX + radius * cos(M_PI * angleSecond * (i + 1) / 100);
    double segmentSecondY = centreY + radius * sin(M_PI * angleSecond * (i + 1) / 100);
    drawLine(segmentFirstX, segmentFirstY, segmentSecondX, segmentSecondY, black);
  }
...
void escapeClicked() {exit(-1);};   // А если Escape??????
